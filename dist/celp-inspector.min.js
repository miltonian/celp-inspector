(function () {
    ...
    function selectElement(element) {
        ...
        try {
            for (const attr of element.attributes) {
                safeElement.attributes[attr.name] = attr.value;
            }
        } catch (e) {
            debug('Error getting attributes:', e);
        }
        ...
        try {
            ...
            debug('Sent selection to parent');
        } catch (e) {
            debug('Error sending to parent:', e);
        }
    ...
    function sendMessage(data) {
        try {
            // Safely serialize any props before sending
            if (data.data && data.data.props) {
                // Convert props to a safe format
                data.data.props = JSON.parse(JSON.stringify(data.data.props));
            }
            ...
            // Send a simplified version if full serialization fails
            window.postMessage({
                source: MESSAGE_PREFIX,
                type: data.type,
                data: {
                    componentName: data.data?.componentName || 'Unknown',
                    simplified: true
                }
            }, '*');
        }
    ...
    function handleClick(e) {
        ...
        debug('Click detected on element:', element.tagName);
